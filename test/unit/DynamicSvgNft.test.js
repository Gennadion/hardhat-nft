	...
	: describe("Dynamic SVG NFT Unit Tests", function () {
			...
			describe("constructor", function () {
				it("initializes the contract correctly", async function () {
					...
					const lowSvgImageUri = await dynamicSvgNft.getLowSvg();
					const highSvgImageUri = await dynamicSvgNft.getHighSvg();
					assert.equal(priceFeed, mockV3Aggregator.address);
					assert.equal(lowSvgImageUri, lowSVGImageuri);
					assert.equal(highSvgImageUri, highSVGimageUri);
				});
			});
			describe("NFT Minting", function () {
				it("emits event on request and creates NFT", async function () {
